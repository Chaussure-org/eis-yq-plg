...
@RestController
@Api(tags = "log管理")
@RequestMapping("/api/v1/master/log")
public class LogController {
        ...
        public RestMessage<DataEntityCollectionDto> firstLoadPager(@RequestBody String json) throws Exception{
            ...
            List<DataEntity> rows = basePagerService.getPagers(columns, tableName, condition, orders, startRowNum, endRowNum);
            DataEntityCollectionDto collectionDto = new DataEntityCollectionDto();
            if (orders.contains("order by createTime  DESC")){
                rows.stream().sorted(new Comparator<DataEntity>() {
                    @Override
                    public int compare(DataEntity o1, DataEntity o2) {
                        if (((Date) o1.getDatas().get("createTime")).getTime() > ((Date) o2.getDatas().get("createTime")).getTime()) {
                            return (int)(((Date) o1.getDatas().get("createTime")).getTime() - ((Date) o2.getDatas().get("createTime")).getTime());
                        }else{
                            return (int)(((Date) o2.getDatas().get("createTime")).getTime() - ((Date) o1.getDatas().get("createTime")).getTime());
                        }
                    }
                });
            }
        ...
        {
            return "id,containerNo,descri,direct,type,paramsContent,success,exception,createTime";
        }
