...
@RestController
@Api("出库订单箱")
@RequestMapping("/api/v1/master/outBoundList")
@CrossOrigin
public class OrderBoxHzController {
    @Autowired
    private IOrderBoxService orderBoxService;
    @Autowired
    private BasePagerService basePagerService;

    @ApiOperation(value = "打印订单箱详情", notes = "打印订单箱详情")
    @PostMapping("/list")
    public RestMessage<OutBoundOrderBoxDto> saveGoods(@RequestBody String json) throws Exception {
        PrologApiJsonHelper helper = PrologApiJsonHelper.createHelper(json);
        String orderBoxNo = helper.getString("orderBoxNo");

        orderBoxService.sendOrderBoxIndToClient(orderBoxNo);

        return RestMessage.newInstance(true, "打印成功", null);
    }
    ...
    {
        return "bcNo,orderBoxNo,supplierNo,supplierName,area";
    }

    private String getTableName()
    {
        return "outboundboxview";
    }
