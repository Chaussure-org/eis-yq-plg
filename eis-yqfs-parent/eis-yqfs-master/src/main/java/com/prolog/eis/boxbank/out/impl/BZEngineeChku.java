...
import com.prolog.eis.model.order.OrderMxBingding;
...
public class BZEngineeChku {
    ...
    private void bindStation(int storeId,String lxNo, ZhanTaiDto zhanTai) throws Exception {
        ...
        try {
            ...
            List<ContainerSub> containerSubInfos = containerSubInfoMapper.findByMap( MapUtils.put("containerNo", lxNo).getMap(), ContainerSub.class);
            lx.setXkKuCunId(storeId);
            lx.setStationId(zhanTai.getZhanTaiId());
            lx.setLiaoXiangNo(lxNo);
            for (ContainerSub subInfo : containerSubInfos) {
                //排除库存为空的货格
                if(subInfo.getCommodityId() == null || subInfo.getCommodityNum() == null || subInfo.getCommodityNum() <= 0)
                    continue;

                HuoGeDto hg = new HuoGeDto();
                hg.setHuoGeNo(subInfo.getContainerSubNo());
                hg.setLiaoXiangNo(lxNo);
                hg.setSpId(subInfo.getCommodityId());
                hg.setSpCount(subInfo.getCommodityNum());
                hg.setLiaoXiang(lx);
                lx.getHuoGeList().add(hg);
            }
    ...
    private boolean BindDingDan(JianXuanDanDto jxd, LiaoXiangDto lx, DingDanDto dd,ZhanTaiDto zhanTai) throws Exception {
        ...
        List<HuoGeDto> hgList = new ArrayList<HuoGeDto>(lx.getHuoGeList());

        while (true) {
            DingDanMxDto ddmx = null;
            HuoGeDto hg = null;
            for (DingDanMxDto ddmxTemp : dd.getDingDanMxList()) {
                for (HuoGeDto hgTemp : hgList) {
                    if (ddmxTemp.getSpId() == hgTemp.getSpId()) {
