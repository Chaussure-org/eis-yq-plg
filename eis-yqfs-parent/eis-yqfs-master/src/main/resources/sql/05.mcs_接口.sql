...
CREATE TABLE `mcs_task`  (
  ...
  `eis_type` int(2) NOT NULL COMMENT 'EIS系统内部类型',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `stock_id_unique`(`stock_id`) USING BTREE COMMENT '托盘编号唯一索引'
...
DROP TABLE IF EXISTS `transmission_task`;
CREATE TABLE `transmission_task` (
  `id` int(11) NOT NULL COMMENT '主键',
  `task_id` varchar(100) NOT NULL COMMENT '任务ID',
  `address` varchar(100) NOT NULL COMMENT '请求位置',
  `type` int(2) NOT NULL COMMENT '任务类型：1：入库:2：出库',
  `container_no` varchar(50) NOT NULL COMMENT '母托盘编号',
  `target` varchar(50) NOT NULL COMMENT '目的位置：目的坐标',
  `task_state` int(11) NOT NULL COMMENT '任务状态(1.完成、2.失败)',
  `send_count` int(11) NOT NULL COMMENT '发送次数',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `err_msg` varchar(2000) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '错误消息',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='MCS输送线发送任务表';


DROP TABLE IF EXISTS `transmission_task_history`;
CREATE TABLE `transmission_task_history` (
  `id` int(11) NOT NULL COMMENT '主键',
  `task_id` varchar(100) NOT NULL COMMENT '任务ID',
  `address` varchar(100) NOT NULL COMMENT '请求位置',
  `type` int(2) NOT NULL COMMENT '任务类型1.入库；2.出库；3.拣选站',
  `container_no` varchar(50) NOT NULL COMMENT '母托盘编号',
  `target` varchar(50) NOT NULL COMMENT '目的位置：目的坐标',
  `task_state` int(11) NOT NULL COMMENT '任务状态(1.完成、2.失败)',
  `send_count` int(11) NOT NULL COMMENT '发送次数',
  ...
  `err_msg` varchar(2000) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '错误消息',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='MCS输送线发送任务历史表';
